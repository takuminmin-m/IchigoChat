...
function tyosei_newbaod(src){//使用できない文字をカット
	var src2=""
	for(var i=0;i<src.length;i++){
		var p=src.slice(i,i+1)
		for(var j=0;j<list.length;j++){
			if(p.indexOf(list.slice(j,j+1))>=0){
				src2=src2+list2.slice(j,j+1)
			}
		}
		for(var j=0;j<list3.length;j++){
			if(p.indexOf(list3.slice(j,j+1))>=0){
				src2=src2+list4.slice(j*2,j*2+2)
			}
		}
	}
	return src2
...
function newboad() {
	fetch(location, {
		...
		body: JSON.stringify({
			"boad_name": tyosei_newboad(document.forms.js.boad_name)
			"come": tyosei_newbaod(document.forms.js.toukou)
			"pass": tyosei_newbaod(document.forms.js.pass)
			"user": tyosei_newbaod(document.forms.js.nam)
		})
	})
}
